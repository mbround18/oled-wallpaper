...
mod physics_unit_tests {
    ...
    fn test_kepler_orbit_position_at_epoch() {
        ...
        assert!((distance - 100.0).abs() < 2.0, "Distance at epoch should be ~100");
    }
    ...
    fn test_kepler_orbit_position_changes_over_time() {
        ...
        assert!((dist_t0 - dist_t1).abs() < 0.5, "Distance should remain constant for circular orbit");
        assert!((dist_t1 - dist_t2).abs() < 0.5, "Distance should remain constant for circular orbit");
    }
    ...
    fn test_kepler_orbit_period() {
        ...
        assert!((pos_start.x - pos_after_period.x).abs() < 0.5, "X position should return after period");
        assert!((pos_start.y - pos_after_period.y).abs() < 0.5, "Y position should return after period");
    }
    ...
    fn test_orbital_velocity_tangent() {
        ...
        assert!(dot_product.abs() < 10.0, "Velocity should be approximately tangent to orbit");
    }
    ...
    fn test_validation_boundary_values() {
        ...
        assert!(body_min.validate().is_ok(), "Should accept minimum valid values");
        ...
        assert!(body_max.validate().is_ok(), "Should accept maximum valid values");
    }
    ...
    fn test_orbit_validation_boundary_values() {
        ...
        assert!(orbit_min.validate().is_ok(), "Should accept minimum valid orbit values");

        // Maximum eccentricity (just under 1.0)
        let mut orbit_max_ecc = Orbit::circular(
            "planet".to_string(),
            "sun".to_string(),
            100.0,
            60.0,
        ...
        assert!(orbit_max_ecc.validate().is_ok(), "Should accept maximum eccentricity");

        // Eccentricity at boundary (should fail)
        let mut orbit_ecc_1 = Orbit::circular(
            "planet".to_string(),
            "sun".to_string(),
            100.0,
            60.0,
        ...
        assert!(orbit_ecc_1.validate().is_err(), "Should reject eccentricity = 1.0");
    }
