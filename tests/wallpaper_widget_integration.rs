...
use oled_wallpaper::widgets::{calendar_text, clock_text, WidgetSystem};
...
fn widget_text_renders_and_remains_in_viewport() {
    ...
    let text = format!(
        "{}\n{}",
        clock_text(&cfg).unwrap_or_default(),
        calendar_text(&cfg).unwrap_or_default()
    );
    assert!(!text.trim().is_empty());
    let p = widgets.clock.pos_px;
