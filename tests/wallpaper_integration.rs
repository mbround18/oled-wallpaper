...
mod wallpaper_integration_tests {
    use oled_wallpaper::renderer::{scene::Scene, scene::SceneEntity, camera::Camera, RenderPipeline};
    ...
    fn test_planet_positions_update() {
        ...
        let initial_pos = if let SceneEntity::CelestialBody { position, .. } = initial_entity {
            position
        } else {
            [0.0, 0.0, 0.0]
        };
        ...
        let final_pos = if let SceneEntity::CelestialBody { position, .. } = final_entity {
            position
        } else {
            [0.0, 0.0, 0.0]
        };
        ...
        assert_ne!(initial_pos, final_pos, "Planet position should update each frame");
    }
    ...
    fn test_render_various_planet_counts() {
        ...
        assert!(renderer.render(&scene).is_ok(), "Should render with multiple planets");
