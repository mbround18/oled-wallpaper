...
#[cfg(test)]
mod config_tests {
    use oled_wallpaper::config::{Config, animation::AnimationConfig};
    ...
    fn test_animation_config_defaults_valid() {
        ...
        assert!(config.validate().is_ok(), "Default animation config should be valid");
    ...
    fn test_valid_custom_config() {
        ...
        assert!(config.validate().is_ok(), "Custom config with valid values should pass");
    }
    ...
    fn test_invalid_planet_speed_too_high() {
        ...
        assert!(config.validate().is_err(), "Should reject planet_speed > 5.0");
    }
    ...
    fn test_invalid_planet_speed_too_low() {
        ...
        assert!(config.validate().is_err(), "Should reject planet_speed <= 0.0");
    }
    ...
    fn test_invalid_color_component() {
        ...
        assert!(config.validate().is_err(), "Should reject color component > 1.0");
    }
    ...
    fn test_invalid_color_component_negative() {
        ...
        assert!(config.validate().is_err(), "Should reject negative color component");
    }
    ...
    fn test_invalid_alpha_zero() {
        ...
        assert!(config.validate().is_err(), "Should reject alpha = 0.0 (fully transparent)");
    }
    ...
    fn test_color_boundary_values() {
        ...
        assert!(config.validate().is_ok(), "Should accept minimum color values");
        
        // Set to maximum valid
        config.sun_color = [1.0, 1.0, 1.0, 1.0];
        assert!(config.validate().is_ok(), "Should accept maximum color values");
    }
    ...
    fn test_multiple_invalid_colors() {
        ...
        assert!(config.validate().is_err(), "Should reject multiple invalid colors");
    }
    ...
    fn test_planet_size_validation() {
        ...
        assert!(config.validate().is_ok(), "Should accept positive planet size");
        ...
        assert!(config.validate().is_err(), "Should reject negative planet size");
    }
    ...
    fn test_full_config_validation_chain() {
        ...
        assert!(config.validate().is_ok(), "Complex valid config should pass");
    }
}
