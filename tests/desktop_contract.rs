...
mod oled_burnin_prevention_tests {
    ...
    fn test_no_static_pixels_over_15_minutes() {
        ...
        assert!(total_frames > 50000, "Should render >50000 frames in 15 minutes at 60 FPS");
        ...
        for frame in 1..=100 {
            ...
            let movement = (
                (position[0] - prev_position[0]).powi(2) +
        ...
        assert!(max_movement > 0.1, "Celestial bodies must move to prevent burn-in");
    }
    ...
    fn test_multiple_moving_elements_prevent_burnin() {
        ...
        assert_ne!(planet1_orbit_radius, planet2_orbit_radius, 
        ...
        assert_ne!(planet1_speed, planet2_speed, "Planets must have different speeds");
    }
}
...
mod animation_performance_tests {
    ...
    fn test_animation_performance_min_30fps() {
        ...
        let max_frame_time = frame_times.iter().copied().fold(f32::NEG_INFINITY, f32::max);
        assert!(max_frame_time < 66.0, 
    ...
    fn test_frame_rate_stability() {
        ...
        assert!((average - target_frame_time).abs() < 1.0,
                "Average frame time {}ms should be within 1ms of {}ms", 
                average, target_frame_time);
    }
}
